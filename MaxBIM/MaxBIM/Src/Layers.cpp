...
void		allocateMemory (void)
{
	...
	selectedInfo.code_state			= new bool [layerInfo.code_name.size ()];
	selectedInfo.dong_state			= new bool [layerInfo.dong_name.size ()];
	selectedInfo.floor_state		= new bool [layerInfo.floor_name.size ()];
	selectedInfo.CJ_state			= new bool [layerInfo.CJ_name.size ()];
	selectedInfo.orderInCJ_state	= new bool [layerInfo.orderInCJ_name.size ()];
	selectedInfo.obj_state			= new bool [layerInfo.obj_name.size ()];
	selectedInfo.subObj_state		= new bool [layerInfo.subObj_name.size ()];

	// 배열 초기화
	for (xx = 0 ; xx < layerInfo.code_name.size () ; ++xx) {
		selectedInfo.code_state [xx] = false;
		layerInfo.code_state [xx] = false;
		layerInfo.code_idx [xx] = 0;
	}
	for (xx = 0 ; xx < layerInfo.dong_name.size () ; ++xx) {
		selectedInfo.dong_state [xx] = false;
		layerInfo.dong_state [xx] = false;
		layerInfo.dong_idx [xx] = 0;
	}
	for (xx = 0 ; xx < layerInfo.floor_name.size () ; ++xx) {
		selectedInfo.floor_state [xx] = false;
		layerInfo.floor_state [xx] = false;
		layerInfo.floor_idx [xx] = 0;
	}
	for (xx = 0 ; xx < layerInfo.CJ_name.size () ; ++xx) {
		selectedInfo.CJ_state [xx] = false;
		layerInfo.CJ_state [xx] = false;
		layerInfo.CJ_idx [xx] = 0;
	}
	for (xx = 0 ; xx < layerInfo.orderInCJ_name.size () ; ++xx) {
		selectedInfo.orderInCJ_state [xx] = false;
		layerInfo.orderInCJ_state [xx] = false;
		layerInfo.orderInCJ_idx [xx] = 0;
	}
	for (xx = 0 ; xx < layerInfo.obj_name.size () ; ++xx) {
		selectedInfo.obj_state [xx] = false;
		layerInfo.obj_state [xx] = false;
		layerInfo.obj_idx [xx] = 0;
	}
	for (xx = 0 ; xx < layerInfo.subObj_name.size () ; ++xx) {
		selectedInfo.subObj_state [xx] = false;
...
void		deallocateMemory (void)
{
	...
	delete []	selectedInfo.code_state;
	delete []	selectedInfo.dong_state;
	delete []	selectedInfo.floor_state;
	delete []	selectedInfo.CJ_state;
	delete []	selectedInfo.orderInCJ_state;
	delete []	selectedInfo.obj_state;
	delete []	selectedInfo.subObj_state;
}
