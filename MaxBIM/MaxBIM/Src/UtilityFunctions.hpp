...
short		makeTemporaryLayer (API_Guid structurualObject, const char* suffix, char* returnedLayerName);	// 객체의 레이어 이름이 01-S로 시작하는 경우 접두사를 05-T로 바꾸고, 하이픈 + 접미사 문자열을 붙인 레이어 이름을 생성한 후 레이어 인덱스와 이름을 리턴함
